...
ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@"$IP" "mkdir -p docker-broken-link-checker-job"
...
scp -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -r * root@"$IP":docker-broken-link-checker-job
ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@"$IP" "cd docker-broken-link-checker-job && ./scripts/install-docker-and-rebuild.sh"
